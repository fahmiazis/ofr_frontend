...
import FormList from '../../components/Ops/FormList'
...
class AjuanBayarOps extends Component {
    ...
    componentDidUpdate() {
        const { isApplist, isRejectList, uploadBukti, detailOps } = this.props.ops
    ...
    getDataKliring = (val, type) => {
        let finalData = ''
        const {dataAll} = this.props.kliring
        const findKliring = dataAll.find(x => x.nama_singkat.toLowerCase() === (!val.bank_tujuan ? val.bank_tujuan.toLowerCase() : ''))
        if (type === 'nama') {
            if (val.bank_tujuan.toLowerCase() === 'bank mandiri') {
                finalData = ''
            } else {
                finalData = !findKliring ? '' : findKliring.nama_singkat
            }
            return finalData
        } else if (type === 'sandi') {
            finalData = !findKliring ? '' : findKliring.sandi_kliring
            return finalData
        }
    }
    ...
    prepSendEmail = async () => {
        ...
        const tempno = {
            ...
            jenis: 'ops',
            tipe: tipe,
            menu: 'Pembayaran Ajuan (Operasional)'
        }
    ...
    mcmExcel = async () => {
        ...
        const ws = workbook.addWorksheet('report ops')
        ...
        [
            ...
            {header: detailOps.length, key: 'c4'},
            {header: nilai, key: 'c5'},
            {header: '', key: 'c6'},
            {header: '', key: 'c7'},
            {header: '', key: 'c8'},
        ...
        detailOps.map(item => { 
            if (glListrik.find((e) => e === parseInt(item.no_coa)) !== undefined && item.tujuan_tf === 'ID Pelanggan') {
                ...
                ws.addRow({
                    ...
                    c11: item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'sandi'),
                    c12: item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'nama'),
                    c13: '',
                    c14: '',
                    c15: '',
                    c16: '',
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) {
            fs.saveAs(
              ...
              `MCM ops ${detailOps.length > 0 && detailOps[0].no_pembayaran}.xlsx`
            );
          });
    }
        ...
        for (let i = 0; i < detailOps.length; i++) {
            ...
            if (glListrik.find((e) => e === parseInt(item.no_coa)) !== undefined && item.tujuan_tf === 'ID Pelanggan') {
                ...
                const data = [
                    ...
                    item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'sandi'),
                    item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'nama'),
                    "",
                    "",
                    "",
                    "",
                    ...
                    <div className="modalFoot ml-3">
                        ...
                        <div className="btnFoot">
                            <ReactHtmlToExcel
                                ...
                                table="table-to-xls"
                                filename={`Data Ops ${moment().format('DD MMMM YYYY')}`}
                                sheet="Dokumentasi"
                                buttonText="Download"
                            />
                ...
                <Modal size="xl" className='modalrinci' isOpen={this.state.rinciAjuan} toggle={this.modalRinciAjuan}>
                    <ModalBody>
                                            ...
                                            <tr>
                                                ...
                                                <th>
                                                    {/* {detailOps.reduce((accumulator, object) => {
                                                        return accumulator + parseInt((object.dpp !== null && object.dpp !== 0 && object.dpp !== '0' && object.dpp !== '') ? object.dpp : object.nilai_buku);
                                                    }, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} */}
                                                </th>
                                                <th></th>
                                                <th></th>
                        ...
                        <Table borderless responsive className="tabPreview mt-4">
                            <thead>
                                <tr>
                                    {ttdOpsList.pembuat && ttdOpsList.pembuat.length > 0 && (
                                        <th className="buatPre">Dibuat oleh,</th>
                                    )}
                                    {ttdOpsList.pemeriksa && ttdOpsList.pemeriksa.length > 0 && (
                                        <th className="buatPre">Diketahui oleh,</th>
                                    )}
                                    {ttdOpsList.mengetahui && ttdOpsList.mengetahui.length > 0 && (
                                        <th className="buatPre">Diketahui oleh,</th>
                                    )}
                                    {ttdOpsList.penyetuju && ttdOpsList.penyetuju.length > 0 && (
                    ...
                    <div className="modalFoot ml-3">
                        <div>
                            ...
                            {/* <Button color="success" className='ml-1' onClick={() => this.printData('formlistops')}>
                                Download Form
                            </Button> */}
                            <FormList />
                        </div>
                ...
                <Modal size="xl" className='modalrinci' isOpen={this.state.openMcm} toggle={this.modalMcm}>
                                        ...
                                        return (
                                                glListrik.find((e) => e === parseInt(detailOps[0].no_coa)) !== undefined && detailOps[0].tujuan_tf === 'ID Pelanggan' ? (
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th className='tabRep'>{item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'sandi')}</th>
                                                        <th >{item.bank_tujuan.toLowerCase() === 'bank mandiri' ? '' : this.getDataKliring(item, 'nama')}</th>
                                                        <th ></th>
                                                        <th ></th>
                                                        <th ></th>
                                                        <th ></th>
                    ...
                    <div className="modalFoot ml-3">
                        <div className="btnFoot">
                            ...
                            data={csvData} 
                            enclosingCharacter={""}
                            filename={`MCM ops ${detailOps.length > 0 && detailOps[0].no_pembayaran}.csv`} 
                                >
                                Download MCM (CSV)
                            </CSVLink>
                        </div>
                ...
                <Modal isOpen={this.state.modalSubmit} toggle={this.openModalSubmit} centered={true}>
                    <ModalBody>
                        <div className={style.modalApprove}>
                            <div>
                                <text>
                                    Anda yakin untuk submit pembayaran ajuan ops    
                                    <text className={style.verif}> </text>
                                    pada tanggal
                                    <text className={style.verif}> {moment().format('DD MMMM YYYY')}</text> ?
                                </text>
...
const mapDispatchToProps = {
    ...
    uploadBuktiBayar: ops.uploadBuktiBayar,
    getDocBayar: ops.getDocBayar,
    submitBuktiBayar: ops.submitBuktiBayar,
