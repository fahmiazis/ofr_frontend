...
class AjuanBayarIkk extends Component {
    ...
    componentDidUpdate() {
        const { isApplist, isRejectList, uploadBukti, detailIkk } = this.props.ikk
    ...
    mcmExcel = async () => {
        const { detailIkk } = this.props.ikk
        const nilai =  detailIkk.reduce((accumulator, object) => {
            return accumulator + parseInt(object.nilai_ajuan);
        }, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")

        const workbook = new ExcelJS.Workbook();
        const ws = workbook.addWorksheet('report ikk')
        ...
        ws.columns = [
            ...
            {header: detailIkk.length, key: 'c4'},
            {header: nilai, key: 'c5'},
            {header: '', key: 'c6'},
            {header: '', key: 'c7'},
            {header: '', key: 'c8'},
        ...
        detailIkk.map(item => { return ( ws.addRow(
            {
                c1: item.norek_ajuan,
                ...
                c9: '',
                c10: item.bank_tujuan === 'Bank Mandiri' ? 'IBU' : 'LBU',
                c11: item.bank_tujuan === 'Bank Mandiri' ? '' : '',
                c12: item.bank_tujuan === 'Bank Mandiri' ? '' : item.bank_tujuan,
                c13: '',
                c14: '',
                c15: '',
                c16: '',
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) {
            fs.saveAs(
              ...
              `MCM ikk ${detailIkk.length > 0 && detailIkk[0].no_pembayaran}.xlsx`
            );
          });
    }
    ...
    prosesModalMcm = () => {
        ...
        const tempData = [
            ["P", moment().format('YYYYMMDD'), "1300015005005", detailIkk.length, nilai, "", "", "", "", "","", "", "", "", "","", "", "", "", "","", "", "", "", "","", "", "", "", "","", "", "", "", "","", "", "", "", "","", "", ""]
        ]
        for (let i = 0; i < detailIkk.length; i++) {
            const item = detailIkk[i]
            const data = [
                item.norek_ajuan,
                ...
                "",
                item.bank_tujuan === 'Bank Mandiri' ? 'IBU' : 'LBU',
                item.bank_tujuan === 'Bank Mandiri' ? '' : '',
                item.bank_tujuan === 'Bank Mandiri' ? '' : item.bank_tujuan,
                "",
                "",
                "",
                "",
    ...
    render() {
        ...
        return (
            <>
                ...
                <Modal size="xl" className='modalrinci' isOpen={this.state.openMcm} toggle={this.modalMcm}>
                    ...
                    <div className="modalFoot ml-3">
                        <div className="btnFoot">
                            ...
                            data={csvData} 
                            enclosingCharacter={""}
                            filename={`MCM ikk ${detailIkk.length > 0 && detailIkk[0].no_pembayaran}.csv`} 
                                >
                                Download MCM (CSV)
                            </CSVLink>
                        </div>
                ...
                <Modal isOpen={this.props.ikk.isLoading || this.props.email.isLoading || this.props.dokumen.isLoading} size="sm">
                        <ModalBody>
                        <div>
                ...
                <Modal isOpen={this.state.modalSubmit} toggle={this.openModalSubmit} centered={true}>
                    <ModalBody>
                        <div className={style.modalApprove}>
                            <div>
                                <text>
                                    Anda yakin untuk submit pembayaran ajuan ikk    
                                    <text className={style.verif}> </text>
                                    pada tanggal
                                    <text className={style.verif}> {moment().format('DD MMMM YYYY')}</text> ?
                                </text>
...
const mapDispatchToProps = {
    ...
    uploadBuktiBayar: ikk.uploadBuktiBayar,
    getDocBayar: ikk.getDocBayar,
    submitBuktiBayar: ikk.submitBuktiBayar,
