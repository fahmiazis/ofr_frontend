...
class VerifKasbon extends Component {
    ...
    componentDidUpdate() {
        const { isApprove, isReject, subRealisasi } = this.props.ops
        if (subRealisasi === false) {
    ...
    getDataTime = async () => {
        ...
        const cekTime1 = time1 === '' ? 'undefined' : time1
        const cekTime2 = time2 === '' ? 'undefined' : time2
        const status = filter === 'all' ? 'all' : level === '2' ? 9 : 10
    ...
    prepSendEmail = async () => {
        ...
        const tipe = 'submit'
        const cekMenu = level === '2' ? 'Verifikasi Finance Realisasi Kasbon (Operasional)' : 'Verifikasi Tax Realisasi Kasbon (Operasional)'
        const tempno = {
            no: detailOps[0].no_transaksi,
    ...
    openDocNewTab = async (val) => {
        ...
        const newWindow = window.open('verifrealkasbon', '_blank', 'noopener,noreferrer')
        this.setState({docCon: false})
        if (newWindow) {
    ...
    render() {
        ...
        const { tipeEmail, dataRinci, type_kasbon, listMut, dataDownload, listReason, dataMenu, listMenu, detailDoc, listOps} = this.state
        ...
        return (
            <>
                    ...
                    <div className={style.backgroundLogo}>
                        <div className={style.bodyDashboard}>
                            ...
                            <div className={style.headMaster}>
                                <div className={style.titleDashboard}>Verifikasi {level === '2' ? 'Finance' : `Tax`} (Realisasi Kasbon)</div>
                            </div>
                ...
                <Modal isOpen={this.props.ops.isLoading || this.props.dokumen.isLoading || this.props.email.isLoading || this.props.notif.isLoading} size="sm">
                        <ModalBody>
                        <div>
